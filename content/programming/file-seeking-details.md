---
title: "FILE seek-ing routine details"
...
by some angle (in degrees). It's an exercise in Igor Zhirkov's book where the reader is to
use bitmap format to write back the rotated image, the input is also expected to be a bitmap image.

Anyhow, the bitmap [file header](#bitmap-file-header) begins with a 2 byte
...
Now, the first approach I decided on was to read the first two bytes and compare those two bytes with `0x42` and `0x4d`
...
that it should be `fseek()`, (at the time, the choice for `rewind()` was not presented to them).
...
Now, it seems that `fseek()` acquires a lock and calls `_IO_seekoff_unlocked()` is calculating 
a bunch of stuff regarding the `backup buffer` it seems. And then finally calling `_IO_SEEKOFF()`.
This `fseek()` call seems to involve calculations and management of a buffer, and locks in order
...
Please [email me](mailto:sysgrammer@proton.me) if you wish to discuss these results with me, I would be delighted for any kind of feedback.
